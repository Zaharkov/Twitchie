...
namespace Twitchiedll.IRC.Events
{
    public class UserNoticeEventArgs
    {
        ...
        public int ResubMonths { get; internal set; }
        public string ResubUserName { get; internal set; }

        public UserNoticeEventArgs(string ircString)
        {
            ...
            foreach (var split in ircString.Split(' '))
            {
                ...
                if (split.StartsWith("@"))
                {
                    foreach (var part in split.TrimStart('@').Split(';'))
                    {
                        ...
                        if (part.Contains("msg-id=resub"))
                        {
                            IsResub = true;
                            continue;
                        }

                        if (part.Contains("msg-param-months="))
                        {
                            if (ResubMonths == 0)
                                ResubMonths = int.Parse(part.Split('=')[1]);
                        ...
                        {
                            if (ResubUserName == null)
                                ResubUserName = part.Split('=')[1];
                        }
                    }
                }
            }
